  ...
  public AddSourceDataset(Map<String, Operation> currOperations) {
    ...
    List<OperationParam> ingestDataFormatParams = ingesterOps.stream().map(Operation::getParams)
        .flatMap(params -> params.stream()
            .filter(param -> OperationConsts.INGEST_DATAFORMAT.equals(param.getKey()))).collect(toList());
    Set<Object> ingestDataFormats = ingestDataFormatParams.stream().flatMap(param->param.getPossibleValues().stream()).collect(toSet());
