...
public class LocalDataEngineClient implements DataEngineClient
{
    ...
    /////////////////////////OBJECT FOR EACH TYPE OF INGESTER. COMBINATION OF AUTOGENERATED PARAMETERS AND CLASSES THAT KNOW HOW TO FGORMAT EACH INGESTER!
                                                  ...
                                                  Map<String, Object> params) throws ApiException
    {
        ...
        OperationSelection selection = createDataIngestOperation(datasets);
    ...
    private OperationSelection createDataIngestOperation(List<DataSetReference> datasets)
    {
        // Build parameters of operation based on form values
        Map<String, Object> params = new HashMap<>();
        List<List<String>> filepaths = getAllNodelistsAndEdgelists(datasets);
        params.put("inputNodelistUris", filepaths.get(0));
        params.put("inputEdgelistUris", filepaths.get(1));
    ...
    private List<List<String>> getAllNodelistsAndEdgelists(List<DataSetReference> references)
    {
        List<String> nodelists = new LinkedList<>();
        List<String> edgelists = new LinkedList<>();

        for (DataSetReference r : references)
        {
            nodelists.add(r.getNodelistLocation());
            edgelists.add(r.getEdgelistLocation());
        }

        return Lists.newArrayList(nodelists, edgelists);
    }
}
